...
- 출력할 목록 지정<br>
        ...
        `<input list="deplist" name="dept"/><br>
        <datalist id="deplist"><br>
            <option value="컴퓨터공학과"></option><br>
            <option value="영어영문학과"></option><br>
            <option value="경영학과"></option><br>
            <option value="사회체육과"></option><br>
        </datalist><br>
        <br/>`
        <hr/>
        <h3>파일 입력 필드</h3>
...
multipe을 하면 여러파일 선택 가능 --><br>
        ...
        `<input type="file" accept="text/*" /><br>`
        `<input type="file" accept=".pdf/" />`<br>
        <br/>
여러개를 쓸 수 있게 공간을 넓게 만들어줌 --><br>
       ` <textarea cols="30" rows="5" <br>name="content">aaaa</textarea><br>`
        <br/>
        <h3>HTML5 입력태그</h3><br>
- 날짜형식 <br>
...
- 범위 슬라이드 <br>
    `<input type="range" name="score" min="-50" <br>max="100" value="0"/><br/>`
...
- 이미지 제출 버튼 : submit 역할도 함<br>
       ` <input type="image" src="../img/apple.png" alt="제출용 이미지 입니다. "/><br/>`
 - 사용자에게 출력은 되지 않지만 form tag 제출 값으로 전송 됨 --><br>
        `< input type="hidden" name="hide" value="숨은값"/><br>`

- 버튼 타입은 따로 지정하지 않으면 기본값이 submit <br>
`< button type="submit">제출< /button><br>
        < button type="button">TEST< /button><br>`
